...
class PermintaanPeminjaman extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $this->db->join('tb_assets', 'tb_assets.id_assets = tb_lend_assets.assets_id');
        // $this->db->join('tb_users', 'tb_users.id_users = tb_lend_assets.users_id');
        $this->db->join('tb_qty_assets', 'tb_qty_assets.id_qty = tb_lend_assets.qty_id');
        $this->db->where(['lend_status' => 0, 'apprvd_y_dept' => 0, 'apprvd_mis_dept' => 0]);
    ...
    public function formAssets($id_lend, $qty_id, $nama)
    {
        ...
        $data['nama'] = $nama;
        $data['qty_id'] = $qty_id;
        // $data['users'] = $this->db->get_where('tb_users', ['id_users' => $users_id])->row_array();
        $data['category'] = $this->db->get_where('tb_qty_assets', ['id_qty' => $qty_id])->row_array();
        ...
        $data['assets']  = $this->db->query($query)->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('templates/sidebar', $data);
    ...
    public function detailPeminjaman($id_assets, $id_lend, $nama)
    {
        ...
        // $data['user'] = $this->db->get_where('tb_users', ['id_users' => $users_id])->result_array();
        $data['trx'] = $this->db->get_where('tb_lend_assets', ['id_lend' => $id_lend])->result_array();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('templates/sidebar', $data);
    ...
    public function trxLend()
    {
        ...
        if ($data['apprvd_y_dept'] == 0 and $data['apprvd_mis_dept'] == 0) {
            ...
            $this->db->set('apprvd_y_dept', $data['apprvd_y_dept'], FALSE);
            $this->db->set('apprvd_mis_dept', $data['apprvd_mis_dept'], FALSE);
            $this->db->where('id_lend', $data['id_lend'], FALSE);
            ...
            $this->db->set('assets_id', $data['assets_id'], FALSE);
            $this->db->set('notes', $data['notes']);
            $this->db->set('apprvd_y_dept', $data['apprvd_y_dept'], FALSE);
            $this->db->set('apprvd_mis_dept', $data['apprvd_mis_dept'], FALSE);
            ...
            $this->db->where('id_lend', $data['id_lend'], FALSE);
            $this->db->update('tb_lend_assets');
            $this->db->set('status_unit', 0, FALSE);
            $this->db->set('idle', 0, FALSE);
            $this->db->where('id_assets', $data['assets_id']);
